	...
	for( int i = -2; i <= roopCount && !progress->isStopped(); i++ ) {
		...
		for( int j = startJ; j <= endJ; j++ ) {
			...
			if( r == -1 && c == -1 ) {		// initialize value
                ...
                ScoreInfo* info11 = index11 < 0 ? nullptr : ( m_scoreArray + index11 );

				int index10 = getIndex( r - 1, c );
                ScoreInfo* info10 = index10 < 0 ? nullptr : ( m_scoreArray + index10 );

				int index01 = getIndex( r, c - 1 );
                ScoreInfo* info01 = index01 < 0 ? nullptr : ( m_scoreArray + index01 );
                ...
                if( info11 != nullptr && info11->valid ) {
                ...
                if( info10 != nullptr && info10->valid ) {
                ...
                if( info01 != nullptr && info01->valid ) {
...
void DynamicPrograming::traceback( void(*pushFun)( int, int )) {
	...
	int col = m_col - 1;
	int index = getIndex( row, col );
    ScoreInfo* info = index < 0 ? nullptr : ( m_scoreArray + index );
    ...
    while( info != nullptr ) {
		...
		if( info->dir == DIR_NONE ) {
            ...
            info = index < 0 ? nullptr : ( m_scoreArray + index );
		}
	}
