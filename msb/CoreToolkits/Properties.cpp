	...
	for( int i = 0; i < size; i++ ) {
		if( !val.empty() ) {
			val.append( "," );
                ...
                val.append( std::to_string(values[ i ]));
	}
...
std::string Properties::replaceString(
	...
	// check parameters
        if( s == nullptr || prefix == nullptr || suffix == nullptr
	...
	bool keyFlag = false;
		...
		if( keyFlag ) {		// search suffix
			char* cc = strstr( c, suffix );
                        if( cc == nullptr ) {		// suffix is not found
				str.append( prefix );
				str += std::string( c );
			}
			...
			keyFlag = false;
                        ...
                        c = ( cc == nullptr ) ? nullptr : ( cc + strlen( suffix ) );
		}
		else {		// search prefix
			char* cc = strstr( c, prefix );
                        if( cc == nullptr ) {		// prefix is not found
				str += std::string( c );
			}
				...
				keyFlag = true;
                        ...
                        c = ( cc == nullptr ) ? nullptr : ( cc + strlen( prefix ) );
		}
	}
