...
DataSet::DataSet( DataGroupNode* group ) {
	m_group = group;
        m_sample = ( m_group == nullptr ) ? nullptr : group->getSample();
}
	...
	for( std::set< DataGroupNode* >::iterator it = groupSet.begin();
		...
		DataGroupNode* g = *it;
			...
			group = g;
                        ...
                        while( group != nullptr && group->getLevel() > g->getLevel() ) {
				group = group->getParentGroup();
			}
                        while( g != nullptr && g->getLevel() > group->getLevel() ) {
				g = g->getParentGroup();
			}

                        while( group != nullptr && g != nullptr && g != group ) {
				group = group->getParentGroup();
				g = g->getParentGroup();
			}

                        if( group == nullptr || g == nullptr ) {
                                return nullptr;
			}
		}
	}
