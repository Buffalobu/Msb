...
PluginManager::PluginManager() {
	// initialize
    m_menu = nullptr;
    m_pluginParser = nullptr;
    m_settingsParser = nullptr;
	...
	// for Context Menu
    m_pSpectrumItem = nullptr;
    m_pSpectrumView = nullptr;
    m_pSpectrumSubView = nullptr;
    m_pChromatogramItem = nullptr;
    m_pChromatogramView = nullptr;
    m_pChromatogramSubView = nullptr;
    m_pGroupItem = nullptr;
    m_pDataMapView = nullptr;
    m_pDataMapSubView = nullptr;
...
PluginManager::~PluginManager() {
	...
	// Context Menu
    if( m_pSpectrumItem != nullptr ) {
		delete m_pSpectrumItem;
	}
    m_pSpectrumItem = nullptr;
	
    if( m_pSpectrumView != nullptr ) {
		delete m_pSpectrumView;
	}
    m_pSpectrumView = nullptr;
	
    if( m_pSpectrumSubView != nullptr ) {
		delete m_pSpectrumSubView;
	}
    m_pSpectrumSubView = nullptr;

    if( m_pChromatogramItem != nullptr ) {
		delete m_pChromatogramItem;
	}
    m_pChromatogramItem = nullptr;

    if( m_pChromatogramView != nullptr ) {
		delete m_pChromatogramView;
	}
    m_pChromatogramView = nullptr;

    if( m_pChromatogramSubView != nullptr ) {
    ...
    if( m_pDataMapView != nullptr ) {
		delete m_pDataMapView;
	}
    m_pDataMapView = nullptr;

    if( m_pDataMapSubView != nullptr ) {
...
void PluginManager::closeLink( const char* path ) {/* LY LY LY
	// create path string
	std::string p = absolutepath( path );
...
PluginFunctionItem* PluginManager::getFunctionItem( const char* type, const char* name, kome::objects::SettingParameterValues* settings ) {
	...
	// search
    PluginFunctionItem* item = nullptr;
    for( unsigned int i = 0; i < items.size() && item == nullptr; i++ ) {
	...
	// parameters
    if( item != nullptr && !p.empty() && settings != nullptr ) {
